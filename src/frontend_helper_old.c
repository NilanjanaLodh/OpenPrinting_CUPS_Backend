...
char *get_resolution(PrinterObj *p)
{
    char *res;
    GError *error = NULL;
...
char *get_orientation(PrinterObj *p)
{
    char *orientation;
    GError *error = NULL;
    ...
    g_message("default orientation: %s", orientation);
    return orientation;
}
char *get_default_color(PrinterObj *p)
{
    ...
    g_message("default color: %s", color);
    return color;
}
...
Option *get_all_printer_options(FrontendObj *f, gchar *printer_name)
{
    PrinterObj *p = g_hash_table_lookup(f->printer, printer_name);
    g_assert_nonnull(p);
    int num_options;
